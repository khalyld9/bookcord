...
/**
 * Shown when a hub table is missing because migration 0003 has not been run
 * against the Supabase project yet — a setup instruction beats a 500.
 */
export function HubSetupNotice({ table }: { table: string }) {
  return (
    <div className="rounded-3xl border border-dashed border-border bg-card/60 p-6 sm:p-8">
      ...
      <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
        ...
        <code className="font-mono text-xs">
          supabase/migrations/0003_student_hub.sql
        </code>{" "}
        in the Supabase SQL editor, then reload this page. It creates the saved
        books and hold request tables along with their row level security
        policies.
      </p>
    </div>
  );
